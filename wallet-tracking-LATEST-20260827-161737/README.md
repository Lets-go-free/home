...
Version: 27.08.2026 16:17:37 CEST · Phase 2k · Build 20260827-161737
...
- Der DAO1 Claim-Scan ist jetzt **walletweit** und nicht mehr von der gerade gewählten NFT/Klassifizierung abhängig.
- Beim vollständigen Initialscan werden alle erkannten `claimReward()`-Calls der Wallet dekodiert und im gemeinsamen `project_nft_claims`-Cache gespeichert.
- `Mining-Bot`, `Trading-Bot`, einzelne NFT usw. sind anschließend Auswahl-/Auswertungsfilter auf diesem gemeinsamen Wallet-Cache.
- Der Scanstand bleibt deshalb bewusst **je Wallet**, nicht je NFT.
- Neuer interner Scan-Typ `claims_wallet_v2`: bestehende Wallets führen nach diesem Update automatisch **genau einen neuen vollständigen Initialscan** durch. Ein manuelles Löschen von `project_scan_state` ist nicht nötig.
- Die bereits vorhandenen Claim-Zeilen bleiben bestehen; Upsert über Tx-Hash verhindert Duplikate.
- Danach laufen Folge-Scans wieder inkrementell mit 250 Block Puffer.
- Keine neue Supabase-Migration in Phase 2k. `005-dao1-claim-cache.sql` bleibt die aktuelle Migration.

## Neu in Phase 2j

- DAO1 Mining-Auswertung kann jetzt nach Datum gefiltert werden: `Von` / `Bis`.
- Zusätzlich gibt es einen separaten NFT-Filter für die bereits geladene Auswertung.
- Filter wirken auf Tabelle, Summen und NFT-Aufschlüsselung.
- Der Claim-Cache bleibt vollständig; für reine Filteränderungen wird **kein neuer Explorer-/RPC-Scan** ausgelöst.
- Mit `Filter zurücksetzen` werden Datum und NFT-Auswertungsfilter geleert.
- Keine neue Supabase-Migration in Phase 2j. `005-dao1-claim-cache.sql` bleibt die aktuelle Migration.
