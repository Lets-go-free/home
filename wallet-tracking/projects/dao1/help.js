...
function renderDAO1Help(){
  ...
  el.innerHTML=`<div class="custom-token-card"><h3 style="margin-top:0">DAO1 / Apertum · Hilfe</h3><p class="note">Diese Hilfe beschreibt ausschließlich die DAO1-/Apertum-Funktionen. Allgemeine WalletTracking-Funktionen stehen unter „❓ Hilfe / Handbuch“.</p></div>
          ...
          <div class="custom-token-card"><h3 style="margin-top:0">Dashboard-Partner &amp; Rewards</h3><p class="note">Im Dashboard werden DAO1- und APTMDAO-Teampartner getrennt ausgewiesen. Die projektweite Partnerzahl dedupliziert nach DID, sobald beide Trees verifizierte Parent-Kanten liefern; die eigenen Root-DIDs zählen nicht als Partner. Rewards und Referral Rewards werden in Originaltoken/-menge dargestellt, nicht in USD.</p></div><div class="custom-token-card"><h3 style="margin-top:0">Bot-Claims</h3><p class="note">Claims werden als eigene DAO1-Transaktionsart ausgewertet und den bekannten Miner-/Bot-NFTs zugeordnet. Der Claim-Bereich besitzt einen NFT-Filter, damit einzelne Miner getrennt geprüft werden können. Neue Miner-/NFT-Zuordnungen sollen aus der On-Chain-Historie erkannt und anschließend in der Projektklassifikation sauber benannt werden.</p></div>
          ...
          <div class="custom-token-card"><h3 style="margin-top:0">Team</h3><p class="note">Der Team-Bereich führt zwei strikt getrennte Untertabs: „Tree DAO1 (alt)“ und „Tree APTMDAO (neu)“. Partner, Ebenen, DIDs und Referral Rewards dürfen zwischen diesen Trees nicht zusammengeführt werden. Die DID ist in beiden Strukturen dieselbe Identität; getrennt werden ausschließlich die Tree-Beziehungen. Eigene Tree-Roots werden automatisch aus den aktuell zu den DAO-Wallets gehörenden DID-NFTs ermittelt. Bei mehreren eigenen DIDs wird je DID ein eigener Root-Baum aufgebaut; der Root-Filter kann alle oder eine einzelne DID anzeigen. Ein DID-NFT kann auf ein anderes Wallet übertragen werden, die historische Tree-Kante wird dadurch nicht automatisch verändert. Beim alten Tree ist die Parent-Beziehung über das DID-Mint-Event <code>TokenMinted(to, tokenId, fid)</code> verifiziert; <code>fid</code> wird als Parent-ID verwendet. Der Button „Tree DAO1 on-chain ermitteln“ liest diese Kanten direkt von Apertum und zeigt nur dekodierbare, verifizierte Beziehungen. Beim neuen APTMDAO-Tree ist die Parent-Beziehung jetzt on-chain über den verifizierten NFT-Mint-Event belegt: topics[1] enthält die neue APTMDAO-ID, topics[2] die Parent-ID und topics[3] die Wallet. Der neue Graph besitzt einen eigenen globalen Supabase-/IndexedDB-Cache und wird mit 24 Block Overlap inkrementell aktualisiert. Der alte DAO1-Tree wird nach erfolgreicher Discovery als hierarchischer Baum analog zum TLN/VOW-Team dargestellt, bis maximal 20 Ebenen. Partnernamen werden innerhalb des DAO-Projekts walletbezogen über die private Alias-Infrastruktur gespeichert, damit dieselbe Wallet im DAO1- und APTMDAO-Tree denselben Namen zeigt. Bestehende DID-Aliase bleiben als Rückwärtskompatibilität lesbar. Zweige können ein-/ausgeklappt werden; die technische Kantentabelle bleibt nur als DEV/Diagnoseansicht. Details je Partner zeigen DID/Wallet und den Mint-Nachweis; Membership, NFTs/Bots, Kaufdatum, Kaufpreis und Referral-Zuordnung werden nur angezeigt, wenn sie on-chain belastbar belegt sind.</p></div>
