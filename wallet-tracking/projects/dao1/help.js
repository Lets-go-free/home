...
function renderDAO1Help(){
  ...
  el.innerHTML=`<div class="custom-token-card"><h3 style="margin-top:0">DAO1 / Apertum · Hilfe</h3><p class="note">Diese Hilfe beschreibt ausschließlich die DAO1-/Apertum-Funktionen. Allgemeine WalletTracking-Funktionen stehen unter „❓ Hilfe / Handbuch“.</p></div>
          ...
          <div class="custom-token-card"><h3 style="margin-top:0">Team</h3><p class="note">Die Standardansicht ist wallet-zentriert: <strong>1 Wallet = 1 Partner</strong>. Ein Wallet kann mehrere DAO1- und/oder APTMDAO-DIDs besitzen; diese werden im gleichen Wallet-Knoten angezeigt, ohne die zugrunde liegenden on-chain Graphen zu vermischen. Die alten separaten DAO1-/APTMDAO-Trees bleiben als Diagnoseansichten erhalten. Ist dieselbe Partner-Wallet in deiner DAO1- und APTMDAO-Downline vorhanden, bestimmt die APTMDAO-Beziehung die Position im kombinierten Baum. Hat ein alter DAO1-Partner im neuen APTMDAO eine andere Upline gewählt, bleibt er nur über seine für dich relevante DAO1-Beziehung in deinem Baum; fremde APTMDAO-Bots werden dadurch nicht deinem Zweig zugerechnet. Eigene Wallets zeigen ihre DAO1-/APTMDAO-DIDs sowie – soweit aus dem Tree bekannt – die jeweilige eigene Upline. Beim alten Tree ist <code>TokenMinted(to, tokenId, fid)</code> der Parent-Nachweis; beim APTMDAO-Tree liefern Mint-Events child, parent und wallet. Neue Apertum-Miner-Käufe tragen die verwendete APTMDAO-DID zusätzlich direkt im Kaufaufruf. Der verifizierte Referenzfall MinerBot #31722 verwendet DID #7315; deren Parent/Upline ist DID #23. Diese direkte Kauf-DID-Evidenz hat Vorrang vor der historischen Ownership-Heuristik. Für ältere Bots bleibt die historisch belegte Besitzlage am Erwerbsblock der Fallback. Bot und DID dürfen unabhängig transferiert werden. Partnerdetails scannen weiterhin nicht den gesamten NFT-Bestand, sondern nur bekannte Bot-Contracts über contract-gefilterte Transferhistorien.</p></div>
