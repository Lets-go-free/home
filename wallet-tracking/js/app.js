// Phase 6.27 · 26.09.2026 18:08:50 CEST: P5 Diagnose-Fix: 6 verbleibende Post-Launch-Claim-Preisfälle werden beim App-Start auch vor Adminrollen-Hydrierung diagnostiziert; Projekt-Subnav im Dark Mode korrigiert. Build 20260926-180850.
// Phase 6.26 · 26.09.2026 17:56:20 CEST: P5: offene native APTM-Claim-Preise werden beim App-Start/DAO1-Refresh nachgezogen; Prelaunch-Fälle dauerhaft markiert; DAO1 Dark-Mode-Tabellen korrigiert. Build 20260926-175620.
// Phase 6.25 · 26.09.2026 17:31:56 CEST: Audit P5 Diagnose: Missing-Claim-Preise werden mit Exact-/Nachbar-/Legacy-Ankern protokolliert; Preislogik unverändert. Build 20260926-173156.
// Phase 6.24 · 26.09.2026 17:17:59 CEST: Audit P5 UI-Fix: Historischer APTM-Stückpreis im Claim-Tab kommt nun aus project_transaction_asset_flows.price_usd; 10 wirklich unbewertete Flows bleiben offen. Build 20260926-171759.
// Phase 6.21 · 26.09.2026 12:12:27 CEST: Audit P5 Schritt 3: sichtbare Claim-, Transaction- und Export-Lesepfade lesen Auszahlungen nur noch aus project_transaction_asset_flows; Legacy Reward-Felder bleiben Alt-/Kompatibilitätsdaten. Build 20260926-121227.
...
const WT_CURRENT_RELEASE = "6.27";
const WT_RELEASE_REGISTRY = Object.freeze({
  "6.23": {
    title: "Korrektur der historischen APTM-Claim-Preisaktualisierung",
    userNotice: true,
    dataImpact: true,
    migrations: ["dao1:native-claim-flow-prices-v1"],
    bullets: [
      "Die in 6.22 gestartete historische USD-Bewertung nativer APTM-Claim-Flows wird korrigiert erneut ausgeführt.",
      "Ein nur zur Anzeige ergänztes Wallet-Adressfeld wird vor dem Supabase-Upsert entfernt; die Tabelle bleibt weiterhin sauber über wallet_id zugeordnet.",
      "Es ist keine SQL-Migration und kein Löschen des Wallets nötig; eine fehlgeschlagene 6.22-Datenmigration bleibt offen und wird automatisch erneut versucht."
    ]
  },
  "6.22": {
    title: "Historische USD-Werte für native APTM-Claims",
    userNotice: true,
    dataImpact: true,
    migrations: ["dao1:native-claim-flow-prices-v1"],
    bullets: [
      "Die bereits kanonisch gespeicherten nativen APTM-Claim-Flows erhalten ihren historischen APTM/USD-Kurs und USD-Wert direkt im Asset-Flow.",
      "Vorhandene historische Transaktionspreise werden zuerst wiederverwendet; nur noch offene Claim-Blöcke werden gesammelt über die bestehende exact-v13 Price-Anchor-Engine bewertet.",
      "Neue native APTM-Claims werden künftig beim Anlegen des Asset-Flows über denselben zentralen Bewertungsweg bewertet; Claim-Mengen und NFT-Zuordnung bleiben unverändert."
    ]
  },
  "6.20": {
  ...
  "5.93": {
    ...
    bullets: [
      "Der Apertum-RPC-Proxy erlaubt jetzt gezielt die für APTMDAO-DID-Owner/Parent benötigten eth_call-Abfragen; andere eth_call-Ziele/Funktionen bleiben gesperrt.",
      "Teilweise abgeschlossene Datenmigrationen können nun als Status partial in Supabase gespeichert werden und werden nicht mehr durch den alten Status-Constraint abgewiesen.",
      "Die NFT-/Ownership-Prüfung wird als Datenversion 4 automatisch erneut ausgeführt; bei technischen Restfehlern bleibt sie offen statt fälschlich als abgeschlossen zu gelten."
    ]
  },
  "5.92": {
  ...
  "5.91": {
    ...
    migrations: ["dao1:nft-store-metadata-v3"],
    bullets: [
      "Apertum-Explorer-Webseiten werden nicht mehr fälschlich als JSON-Metadatenquelle geladen.",
      "Nur echte Metadata-/Token-URIs werden für Bild und NFT-Namen abgefragt; generische Botnamen bleiben weiterhin nur Fallback.",
      "Die NFT-Metadatenprüfung wird einmalig automatisch wiederholt. Bestehende Besitz-, Kaufpreis-, Spam- und Safe-Daten bleiben erhalten."
    ]
  },
  "5.90": {
    title: "DAO-NFT-Namen werden genauer erkannt",
    userNotice: true,
    dataImpact: true,
    migrations: ["dao1:nft-store-metadata-v3"],
    bullets: [
      "Apertum-NFTs werden erneut auf den echten DAO-Store-/Metadata-Namen geprüft (z. B. Nebula, Galactic, Lunar oder Solar).",
      "Neben dem NFT-Namen werden jetzt auch Store-/Metadata-Attribute und vorhandene DAO-Projektbezeichnungen ausgewertet; generische Bezeichnungen wie MineBot #12345 bleiben nur Fallback.",
      "Bestehende NFT-Caches werden automatisch aktualisiert; Bild, NFT-ID, Kauf-/Besitzhistorie und manuelle Spam-/Safe-Markierungen bleiben erhalten."
    ]
  }
});
...
const WT_DATA_MIGRATIONS = Object.freeze({
  "dao1:native-claim-flow-prices-v1": {version:1,release:"6.22",label:"DAO1 native APTM-Claim-Flows → historische USD-Bewertung",run:wtMigrationDao1NativeClaimFlowPrices},
...
async function onLoggedIn(session) {
  ...
  // Unabhängige Start-Reads parallel ausführen. Das Dashboard bleibt währenddessen sichtbar.
  // Dieses Muster entspricht den Projekt-Trees: kleiner Startkern, Cache/DB parallel, keine Discovery.
  loadTokenMetaCache();
  const [adminResult]=await Promise.all([
    checkIsAdmin(),
    loadDefiProjectsCache(),
    loadPredefinedTokensFromDb(),
    loadWalletsFromDb(),
  ...
  if (isAdmin) document.getElementById("newPredefChain").innerHTML = Object.keys(CHAIN_META).map(c => `<option value="${c}">${CHAIN_META[c].label}</option>`).join("");
  setupChatRealtime();
  // Badge ist für den Start nicht kritisch und blockiert den Dashboard-Aufbau nicht mehr.
  updateChatUnreadBadge().catch(e=>console.warn("Chat-Badge",e));
...
const ADMIN_SYSTEM_TREE = [
  ...
  {id:"dao-help",level:2,label:"Hilfe",status:"done",start:"–",daily:"–",open:"lokal",manual:"–",details:[["DAO1 Hilfe","JS-Modul","–","–","Tab öffnen"]]},

  {id:"cache-audit",level:0,label:"⚡ Cache-/Request-Audit",status:"done",idea:"Cache-/Request-Audit und Startoptimierung",start:"🟢 Session-Audit + kein Auto-loadAll",daily:"kein Blind-Refresh",open:"Tab-Marker + Messung",manual:"gezielte Vergleichsläufe",details:[
    ["Request-Inventar","Session/RAM","Supabase Tabellen + cache_data_versions","RPC/API je tatsächlich ausgeführtem Call","Phase 5.80 Abschlussstand: TLN/VOW bündelt die Wallet-Token-Prüfung projektweit in einem Multicall, lädt Discovery-Snapshots aller eigenen Wallets in einem DB-Batch und memoisiert technische Cache-Reads pro Session. Der normale TLN-Haupttab initialisiert DEX/Provider nicht mehr; Kurse/Pools tun dies erst beim Untertab. DAO-Partner-Current-Holdings werden 24h im zentralen IndexedDB-Cache wiederverwendet. DATA_VERSIONS bleibt in-flight dedupliziert und 30 s gecacht; Start-/Tab-/Refresh-Marker segmentieren den Lauf"],
    ...
    ["Historisches ownerOf@Block","persistenter Ergebnis-Cache","DAO Lifecycle/History Cache","gezielter eth_call über geeigneten RPC","Separat reparieren; darf aktuelle Bot-Anzahl nicht beeinflussen"]]},
...
const LIFECYCLE_ARCH_AUDIT_ITEMS = [
  ...
  {priority:"P5", workStatus:"in Arbeit", severity:"critical", area:"DAO1 Claims / Payouts", finding:"Der Audit zeigt drei überlappende Persistenz-/Readmodelle: project_transaction_asset_flows enthält den tatsächlichen Assetfluss; project_nft_claims speichert Claim→NFT plus Reward-Assetfelder; project_transactions enthält zusätzlich Claim-Marker und Legacy reward_aptm/USD-Felder. Das ist funktional, kann aber widersprüchliche Auszahlungswerte erzeugen.", action:"Phase 6.19 hat alle sichtbaren Claim-Auszahlungen asset-flow-first vereinheitlicht. Phase 6.20 persistiert bestätigte native APTM-Claims als kanonische project_transaction_asset_flows und migriert bestehende native Claim-Caches einmalig ohne Blockchain-Vollscan; der Realtest bestätigte 263 eindeutige native Claim-Flows / 263 TX / 1’829.869891 APTM. Phase 6.21 entfernt die Legacy-Reads aus sichtbaren Claim-, Transaktions- und Exportpfaden. Phase 6.22 bewertet native APTM-Claim-Flows kanonisch mit historischen USD-Werten: vorhandene Transaktionspreise werden zuerst wiederverwendet, nur offene Blöcke laufen gesammelt über die bestehende exact-v13 Price-Anchor-Engine; neue native Claims nutzen denselben Bewertungsweg. Der 6.22-Realtest deckte PGRST204 auf: hydratisiertes wallet_address war kein DB-Feld von project_transaction_asset_flows. Phase 6.23 sanitisiert Asset-Flow-Upserts zentral auf das Persistenzschema; die Preis-Migration lief im Realtest erfolgreich: 267 Flows, davon 257 historisch bewertet und 10 bewusst offen. Phase 6.24 liest nun auch die sichtbare Stückpreis-Spalte im Claim-Tab aus Asset-Flow price_usd; claim_reward_aptm/USD bleiben nur Alt-/Kompatibilitätsdaten. Der Realtest zeigte, dass mehrere der 10 Missing-Fälle deutlich nach dem APTM-Marktstart liegen. Phase 6.25 ergänzt eine Anchor-Diagnose (Exact-Anchor, gültiger Nachbar davor/danach, Legacy-Predecessor). Phase 6.26 verlagert Repricing und Diagnose in App-Start/DAO1-Aktualisierung, überspringt dauerhaft markierte Prelaunch-Fälle und behebt zugleich den DAO1-Dark-Mode-Zebrafehler. Der Realtest identifizierte 4 echte Prelaunch-Fälle und 6 Post-Launch-Missing-Fälle, aber diagnostics=0, weil der Adminstatus beim frühen Startjob noch nicht verfügbar war. Phase 6.27 behebt ausschließlich diesen Diagnose-Timingfehler und den noch hellen Projekt-Subnav-Container im Dark Mode; die Preislogik bleibt unverändert. Danach ist die Ursache der 6 verbleibenden Marktpreis-Lücken gezielt zu beheben."},
...
function lifecycleWorkStatusBadge(i){
  if(i.workStatus==="in Arbeit")return '<span class="badge" style="background:rgba(70,130,220,.15);color:#4682dc">in Arbeit</span>';
  if(i.workStatus==="erledigt")return '<span class="badge safe">erledigt</span>';
  if(i.workStatus==="beobachten")return '<span class="badge" style="background:rgba(120,120,120,.14);color:var(--muted)">beobachten</span>';
  return '<span class="badge unsafe">offen</span>';
}
function renderHardcodingAudit(){
  ...
  const lifecycleRows=LIFECYCLE_ARCH_AUDIT_ITEMS.map(i=>`<tr><td><strong>${escapeAttr(i.priority)}</strong></td><td>${lifecycleWorkStatusBadge(i)}</td><td>${lifecycleAuditBadge(i)}</td><td>${escapeAttr(i.area)}</td><td>${escapeAttr(i.finding)}</td><td>${escapeAttr(i.action)}</td></tr>`).join("");
  ...
  el.innerHTML=`
    <div class="custom-token-card" style="margin-bottom:16px">
      <h3 style="margin-top:0">Lifecycle-/Architektur-Audit · Phase 6.09</h3>
...
async function removeCustomSafeToken(chain, address) {
  ...
  const label = t?.label || address;
  if (!confirm(`Token "${label}" (${String(chain || "").toUpperCase()}) wirklich aus deiner sicheren Token-Liste entfernen?`)) return;
  if (t && t.dbId) {
    const { error } = await sb.from("safe_tokens").delete().eq("id", t.dbId);
...
// Globaler, nicht personenbezogener Snapshot für aktuelle Preise.
...
function priceSlotKey(value=new Date()){
  ...
  const parts=new Intl.DateTimeFormat("en-CA",{timeZone:CURRENT_PRICE_TIMEZONE,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23"}).formatToParts(d);
  const x=Object.fromEntries(parts.filter(p=>p.type!=="literal").map(p=>[p.type,p.value]));
  const minute=Math.floor(Number(x.minute||0)/CURRENT_PRICE_SLOT_MINUTES)*CURRENT_PRICE_SLOT_MINUTES;
  return `${x.year}-${x.month}-${x.day}T${x.hour}:${String(minute).padStart(2,"0")}`;
}
function nextPriceSlotDelayMs(){
  const now=Date.now(),d=new Date(now),m=d.getMinutes(),next=(Math.floor(m/CURRENT_PRICE_SLOT_MINUTES)+1)*CURRENT_PRICE_SLOT_MINUTES;
  const n=new Date(d);n.setSeconds(2,0);if(next>=60){n.setHours(d.getHours()+1,0,2,0);}else n.setMinutes(next,2,0);
  return Math.max(1000,n.getTime()-now);
}
...
async function loadGlobalCurrentPriceSnapshot(){
  try{
    const {data,error}=await sb.from(CURRENT_PRICE_GLOBAL_CACHE_TABLE).select("captured_at,payload,slot_key").eq("snapshot_key","current").eq("valuation_version",CURRENT_PRICE_GLOBAL_CACHE_VERSION).maybeSingle();
    if(error)throw error;if(!data?.payload)return null;
    return {capturedAt:data.captured_at,payload:data.payload,slotKey:data.slot_key,currentSlot:data.slot_key===priceSlotKey()};
  }catch(e){console.warn("Globaler Preiscache konnte nicht aus Supabase geladen werden",e);return null;}
}
function hydrateGlobalCurrentPriceSnapshot(payload){
...
async function saveGlobalCurrentPriceSnapshot(slotKey=priceSlotKey()){
  const capturedAt=new Date().toISOString(),payload={nativePrices,tokenPrices};
  try{
    const {error}=await sb.from(CURRENT_PRICE_GLOBAL_CACHE_TABLE).upsert({snapshot_key:"current",valuation_version:CURRENT_PRICE_GLOBAL_CACHE_VERSION,slot_key:slotKey,captured_at:capturedAt,payload,updated_at:capturedAt},{onConflict:"snapshot_key,valuation_version"});
    if(error)throw error;currentPriceCacheState={capturedAt,source:"live",slotKey};return true;
  }catch(e){console.warn("Globaler Preiscache konnte nicht gespeichert werden",e);currentPriceCacheState={capturedAt,source:"live-unsaved",slotKey};return false;}
}
async function claimGlobalPriceSlot(slotKey=priceSlotKey()){
  try{
    const {data,error}=await sb.rpc("wallettracking_claim_price_refresh_slot",{p_slot_key:slotKey,p_valuation_version:CURRENT_PRICE_GLOBAL_CACHE_VERSION});
    if(error)throw error;return data===true;
  }catch(e){console.warn("Globaler Preis-Slot konnte nicht reserviert werden",e);return false;}
}
async function loadCachedCurrentPricesAtStart(){
  const [cached]=await Promise.all([
    loadGlobalCurrentPriceSnapshot(),
    window.TLNVOWProject?.loadCachedPrices?.().catch?.(e=>{console.warn("TLN/VOW Preissnapshot",e);return null;}) || Promise.resolve(null)
  ]);
  if(cached?.payload&&hydrateGlobalCurrentPriceSnapshot(cached.payload)){
    currentPriceCacheState={capturedAt:cached.capturedAt,source:"supabase",slotKey:cached.slotKey};
    setCentralPriceStatus(`${formatCurrentPriceTimestamp(cached.capturedAt)} · global gespeichert${cached.currentSlot?" · aktueller 15-Min.-Slot":""}.`,cached.currentSlot?"success":"warning");
    setWtDataStatus("dashboard",{updatedAt:cached.capturedAt,cacheAt:cached.capturedAt,source:"cache",label:"Dashboard"});
    rerenderAllCurrentPriceViews();renderDashboard();return currentPriceCacheState;
  }
...
async function refreshAllCurrentPrices({manual=false}={}){
  ...
  allCurrentPricesPromise=(async()=>{
    const slotKey=priceSlotKey();
    if(!manual){
      const cached=await loadGlobalCurrentPriceSnapshot();
      if(cached?.currentSlot&&hydrateGlobalCurrentPriceSnapshot(cached.payload)){
        currentPriceCacheState={capturedAt:cached.capturedAt,source:"supabase",slotKey:cached.slotKey};
        await window.TLNVOWProject?.loadCachedPrices?.();rerenderAllCurrentPriceViews();
        setCentralPriceStatus(`${formatCurrentPriceTimestamp(cached.capturedAt)} · global gespeichert · aktueller 15-Min.-Slot.`,`success`);return {general:currentPriceCacheState,tln:window.TLNVOWProject?.getPriceState?.()||null};
      }
      const claimed=await claimGlobalPriceSlot(slotKey);
      if(!claimed){
        // Ein anderer aktiver Client aktualisiert gerade. Kurz darauf den globalen Snapshot übernehmen.
        await new Promise(r=>setTimeout(r,2500));const after=await loadGlobalCurrentPriceSnapshot();
        if(after?.payload&&hydrateGlobalCurrentPriceSnapshot(after.payload)){currentPriceCacheState={capturedAt:after.capturedAt,source:"supabase",slotKey:after.slotKey};await window.TLNVOWProject?.loadCachedPrices?.();rerenderAllCurrentPriceViews();setCentralPriceStatus(`${formatCurrentPriceTimestamp(after.capturedAt)} · global gespeichert.`,`success`);return {general:currentPriceCacheState,tln:window.TLNVOWProject?.getPriceState?.()||null};}
        return null;
      }
    }
...
function scheduleGlobalPriceRefresh(){
  if(globalPriceTimer)clearTimeout(globalPriceTimer);
  const tick=async()=>{if(currentUser)await refreshAllCurrentPrices({manual:false}).catch(e=>console.warn("Globaler 15-Minuten-Preisjob",e));globalPriceTimer=setTimeout(tick,nextPriceSlotDelayMs());};
  // Beim Login aktuellen Slot prüfen; danach an :00/:15/:30/:45 (mit 2 s Sicherheitsabstand).
  setTimeout(()=>{if(currentUser)refreshAllCurrentPrices({manual:false}).catch(e=>console.warn("Globaler Preisstart",e));},300);
  globalPriceTimer=setTimeout(tick,nextPriceSlotDelayMs());
}
document.addEventListener("visibilitychange",()=>{if(!document.hidden&&currentUser)refreshAllCurrentPrices({manual:false}).catch(e=>console.warn("Preisstand nach Rückkehr",e));});
...
function isSupabaseAuthStorageKey(key) {
  const k=String(key||"");
  return /^sb-[^-]+-auth-token$/i.test(k) || /supabase.*auth.*token/i.test(k);
}
function clearWebStorageExceptAuth(storage,preserveAuthSession=false){
  try{
    if(!preserveAuthSession){storage.clear();return;}
    const keep=new Map();
    for(let i=0;i<storage.length;i++){
      const key=storage.key(i);
      if(key && isSupabaseAuthStorageKey(key))keep.set(key,storage.getItem(key));
    }
    storage.clear();
    for(const [key,value] of keep)if(value!=null)storage.setItem(key,value);
  }catch(_){}
}
async function clearWalletTrackingLocalUserData({preserveAuthSession=false}={}) {
  try {
  ...
  clearWebStorageExceptAuth(localStorage,preserveAuthSession);
  clearWebStorageExceptAuth(sessionStorage,preserveAuthSession);
}
...
async function deleteAllWalletTrackingUserData() {
  ...
  try {
    ...
    await clearWalletTrackingLocalUserData({preserveAuthSession:keepAdminSession});

    if(keepAdminSession){
      alert("Alle gespeicherten WalletTracking-Daten wurden vollständig gelöscht. Admin-Testmodus: Du bleibst angemeldet und startest jetzt mit leerem WalletTracking neu.");
      location.reload();
    }else{
...
function clearWalletRelatedMemory(w) {
  ...
  const localId = String(w?.id || "");
  if (localId) delete walletData[localId];

  nftCaches?.delete?.(walletId);
  discoveryCaches?.delete?.(walletId);
  if (String(currentDiscoveryWalletId?.() || "") === walletId) {
    discoveryCache = null;
    lastDiscoveryFindings = [];
  }

  if (typeof feeCaches !== "undefined" && feeCaches?.keys) {
    for (const key of [...feeCaches.keys()]) if (String(key).startsWith(walletId + "|")) feeCaches.delete(key);
  }
...
async function initializeSavedWalletTargeted(w,{isNew=false}={}) {
  ...
  const started=performance.now(),failures=[],timings={};
  const timed=async(name,fn)=>{
    const t0=performance.now();
    try{return await fn();}
    finally{timings[name]=Math.round(performance.now()-t0);}
  };
  ...
  for(const chain of chains){
    ...
    try{
      ...
      const oldMap=new Map(((old?.nfts)||[]).map(n=>[nftKey(n),{
        ...
        name:n.name||null,nameSource:n.nameSource||null,collectionName:n.collectionName||null,purchaseEvidence:n.purchaseEvidence||null
      }]));
      found.forEach(n=>{
        ...
        if(f.name)n.name=bestNftName(n.tokenId,n.collectionName||f.collectionName,n.name,f.name);if(!n.nameSource&&f.nameSource)n.nameSource=f.nameSource;if(!n.collectionName&&f.collectionName)n.collectionName=f.collectionName;
...
function dashboardRewardPeriodHtml(rows,assetOrder=null){const list=Array.isArray(rows)?rows:[];if(!assetOrder?.length){if(!list.length)return "–";return `<span class="dashboard-reward-assets">${list.map(x=>`<span><strong>${dashboardRewardAmount(x.amount,x)}</strong> ${escapeAttr(x.symbol||x.name||"TOKEN")}</span>`).join("")}</span>`;}const map=new Map(list.map(x=>[dashboardRewardAssetKey(x),x]));return `<span class="dashboard-reward-assets">${assetOrder.map(a=>{const x=map.get(a.key);return `<span><strong>${x?dashboardRewardAmount(x.amount,x):"–"}</strong> ${escapeAttr(a.symbol||"TOKEN")}</span>`}).join("")}</span>`;}
function dashboardRewardAssetOrder(group){const map=new Map();for(const period of ["total","previousYear","year","month"]){for(const x of (Array.isArray(group?.[period])?group[period]:[])){const key=dashboardRewardAssetKey(x);if(key&&!map.has(key))map.set(key,{key,symbol:x.symbol||x.name||"TOKEN"});}}return [...map.values()].sort((a,b)=>String(a.symbol).localeCompare(String(b.symbol)));}
...
function setDashboardProjectCacheStats(projectKey,patch={}){
  ...
  for(const field of ["dao1Partners","aptmdaoPartners","expiredPartnerStakings","upcomingPartnerStakings","recentPartnerActivities"])if(Object.prototype.hasOwnProperty.call(patch,field))cur[field]=patch[field];
...
function renderDashboard(){
  ...
  const projectCards=[...involvedProjects].map(key=>[key,portfolio.projects.get(key)||{valueUsd:0,freeUsd:0,boundUsd:0,assets:0}]).map(([key,p])=>{
    ...
    const teamStats=key==="dao1"?`<div><span>Partner-Wallets</span><strong>${dashboardMetric(stats.teamPartners)}</strong></div><div><span>DAO1 Bezug</span><strong>${dashboardMetric(stats.dao1Partners)}</strong></div><div><span>APTMDAO Bezug</span><strong>${dashboardMetric(stats.aptmdaoPartners)}</strong></div>`:`<div><span>Teampartner</span><strong>${dashboardMetric(stats.teamPartners)}</strong></div><div><span>davon aktiv</span><strong>${dashboardActivePartners(stats)}</strong></div>`;
  ...
  const rewardKpi=(title,group)=>{const order=dashboardRewardAssetOrder(group);return `<article class="dashboard-kpi dashboard-kpi-gold dashboard-reward-kpi"><span>${title}</span><div class="dashboard-kpi-reward-lines"><div><small>Gesamt</small>${dashboardRewardPeriodHtml(group.total,order)}</div><div><small>Vorjahr</small>${dashboardRewardPeriodHtml(group.previousYear,order)}</div><div><small>Jahr</small>${dashboardRewardPeriodHtml(group.year,order)}</div><div><small>Monat</small>${dashboardRewardPeriodHtml(group.month,order)}</div></div></article>`;};
...
async function renderProjectLpTab(projectKey,chains,targetId,dateStr="2025-12-31",refresh=false,options={}){
    ...
    for(const w of scopeWallets){
      ...
      if(!refresh){
        ...
        try{
          ...
          // Historische LP-Wallets dürfen nicht verschwinden, nur weil ein älterer Lauf
          // keinen lp_position_cache-Eintrag erzeugt hat oder der heutige Bestand 0 ist.
          // In diesem Fall wird aus lp_history_events eine reine Historienzeile aufgebaut.
          const historicalPairs=new Map();
          for(const e of sync.events){
            const pa=normalizeAddress(e.pair_address,chain);if(!pa||cachedPairSet.has(pa)||historicalPairs.has(pa))continue;
            historicalPairs.set(pa,e);
          }
          for(const [pa,e] of historicalPairs){
            rows.push({
              chain,w,historicalOnly:true,
              pair:{address:pa,t0:{address:e.token0_address,symbol:e.token0_symbol||'Token0',decimals:e.token0_decimals},t1:{address:e.token1_address,symbol:e.token1_symbol||'Token1',decimals:e.token1_decimals}},
              cur:{balance:0,walletBalance:0,stakedBalance:0,amount0:0,amount1:0,share:0,usd:null},
              histBal:null,histWalletBal:null,histStakedBal:null,histUsd:null,cached:true
            });
          }
  ...
  const availableWalletIds=[...new Set([
    ...rows.map(r=>walletDbId(r.w)),
    ...history.map(r=>walletDbId(r.w)),
    ...scanStatuses.filter(s=>Number(s.eventCount||0)>0||Number(s.positionCount||0)>0).map(s=>walletDbId(s.w))
  ].filter(Boolean))];
...
function nftMetadataNameCandidates(meta,depth=0) {
  ...
  const add=(value,source,priority=0)=>{
    ...
    if(!v)return;
    out.push({value:v,source,priority});
  };
  add(meta.name,"metadata.name",100);
  add(meta.title,"metadata.title",96);
  add(meta.display_name ?? meta.displayName,"metadata.display_name",94);
  add(meta.bot_name ?? meta.botName,"metadata.bot_name",98);
  add(meta.model_name ?? meta.modelName,"metadata.model_name",92);
  add(meta.variant_name ?? meta.variantName,"metadata.variant_name",90);
  const props=meta.properties&&typeof meta.properties==="object"?meta.properties:null;
  if(props){
    add(props.name,"metadata.properties.name",95);
    add(props.title,"metadata.properties.title",93);
    add(props.bot_name ?? props.botName,"metadata.properties.bot_name",97);
    add(props.model_name ?? props.modelName,"metadata.properties.model_name",91);
  }
  const attrs=Array.isArray(meta.attributes)?meta.attributes:[];
  for(const a of attrs){
    if(!a || typeof a!=="object")continue;
    const key=String(a.trait_type ?? a.traitType ?? a.name ?? a.key ?? a.title ?? "").trim();
    const value=a.value ?? a.display_value ?? a.displayValue ?? a.val;
    if(value==null)continue;
    const k=key.toLowerCase().replace(/[_-]+/g," ").replace(/\s+/g," ").trim();
    let priority=0;
    if(/^(bot )?name$/.test(k))priority=99;
    else if(/^(bot )?model( name)?$/.test(k))priority=94;
    else if(/^(bot )?variant( name)?$/.test(k))priority=92;
...
function bestMetadataStoreName(tokenId,collectionName,...metas){
  const candidates=[];
  for(const meta of metas)candidates.push(...nftMetadataNameCandidates(meta));
  candidates.sort((a,b)=>b.priority-a.priority);
  for(const c of candidates){
    if(nftNameQuality(c.value,tokenId,collectionName)>=3)return c;
  }
  return null;
}

let apertureProjectNftNameCache={loadedAt:0,map:new Map()};
async function loadApertureProjectNftNameMap(force=false){
  const age=Date.now()-Number(apertureProjectNftNameCache.loadedAt||0);
  if(!force && age<300000 && apertureProjectNftNameCache.map instanceof Map)return apertureProjectNftNameCache.map;
  const map=new Map();
  try{
    const {data,error}=await sb.from("project_nfts").select("nft_contract,nft_id,nft_name").eq("project_key","dao1").eq("chain_key","apertum").eq("enabled",true);
    if(error)throw error;
    for(const r of (data||[])){
      const contract=lowerAddressForNft(r.nft_contract),id=String(r.nft_id??"");
      const name=String(r.nft_name||"").trim();
      if(contract&&id&&name)map.set(`${contract}|${id}`,name);
    }
  }catch(e){
    if(!/does not exist|schema cache/i.test(String(e?.message||"")))console.warn("DAO-NFT-Projektnamen:",e);
  }
  for(const o of (nftOwnershipRows||[])){
    const contract=lowerAddressForNft(o?.nft_contract),id=String(o?.nft_id??"");
    const name=String(o?.nft_name||"").trim();
    if(!contract||!id||!name)continue;
    const key=`${contract}|${id}`,old=map.get(key)||"";
    map.set(key,bestNftName(id,"",old,name));
  }
  apertureProjectNftNameCache={loadedAt:Date.now(),map};
  return map;
}
async function applyApertureCanonicalNames(nfts){
  if(!Array.isArray(nfts)||!nfts.length)return nfts||[];
  const names=await loadApertureProjectNftNameMap();
  for(const n of nfts){
    const key=`${lowerAddressForNft(n?.tokenAddress)}|${String(n?.tokenId??"")}`;
    const canonical=names.get(key);
    if(!canonical)continue;
    const chosen=bestNftName(n.tokenId,n.collectionName,n.name,canonical);
    if(chosen!==n.name){n.name=chosen;n.nameSource="dao-project-nft";}
  }
  return nfts;
}
...
async function enrichApertumNft(chain, nft) {
  ...
  try {
    ...
    const collectionName=nft.collectionName||token.name||token.symbol;
    let external=null;
    let image=nft.image || normalizeNftImageUrl(inst?.image_url||inst?.media_url) || nftMetadataImage(meta);
    let storeCandidate=bestMetadataStoreName(nft.tokenId,collectionName,meta);
    let preferredName=bestNftName(nft.tokenId,collectionName,
      storeCandidate?.value,meta?.name,inst?.name,nft.name,token.name,token.symbol);
    if(!image || nftNameQuality(preferredName,nft.tokenId,collectionName)<3){
      const uri=inst?.metadata_url||inst?.token_uri||inst?.tokenUri||token?.metadata_url||token?.token_uri||null;
      external=uri?await fetchExternalNftMetadata(uri):null;
      if(external){
        meta={...external,...meta};
        image=image || nftMetadataImage(external);
        storeCandidate=bestMetadataStoreName(nft.tokenId,collectionName,meta,external)||storeCandidate;
        preferredName=bestNftName(nft.tokenId,collectionName,
          storeCandidate?.value,meta?.name,external?.name,inst?.name,nft.name,token.name,token.symbol);
      }
    }
    let out={
      ...nft,
      name:preferredName,
      nameSource:storeCandidate?.value===preferredName?(storeCandidate?.source||"apertum-store-metadata"):(nft.nameSource||null),
...
async function fetchApertumNfts(chain,address,onProgress){
  ...
  const mergeMs=Math.round(performance.now()-mark);
      ...
      for(let off=0;off<ids.length;off+=100){
        ...
        for(const r of (data||[])){
          ...
          const candidate={
            ...
            wallet_address:r.to_address||null
          };
...
function nftOwnershipInfo(n){
  if(String(n?.chain||"")!=="apertum"){
    ...
    return at||block?{known:true,firstOwnedAt:at,firstOwnedBlock:block,firstOwnedWalletId:String(n?.walletId||n?.wallet_id||""),firstOwnedWalletAddress:n?.walletAddress||null,walletSinceAt:at,walletSinceBlock:block,currentWalletId:String(n?.walletId||n?.wallet_id||""),currentInWallet:true,source:n?.acquisitionSource||"incoming-nft-transfer"}:{known:false};
  }
  const contract=lowerAddressForNft(n?.tokenAddress);
  const id=String(n?.tokenId??"");
...
function nftWalletAddressById(walletId,chain="apertum"){
  const w=wallets.find(x=>String(x.id||x.dbId||"")===String(walletId||""));
  return w?walletAddressForChain(w,chain):null;
}
...
async function refreshApertumNftsForWallet(wallet,onProgress=null){
  ...
  const fetchMs=Math.round(performance.now()-mark);
  mark=performance.now();
  ...
  const mergeFlagsMs=Math.round(performance.now()-mark);
  mark=performance.now();
  await saveNftCacheForWallet(wallet,others.concat(found),[...new Set([...(old?.selected_chains||[]),chain])]);
  const saveCacheMs=Math.round(performance.now()-mark);
  const totalMs=Math.round(performance.now()-perfStarted);
  console.info(`DAO1 NFT Inventory Performance · wallet=${wallet.label||wallet.id||""} · fetchMs=${fetchMs} · mergeFlagsMs=${mergeFlagsMs} · saveCacheMs=${saveCacheMs} · totalMs=${totalMs} · found=${found.length} · priorCached=${Array.isArray(old?.nfts)?old.nfts.length:0}`);
  return found;
}
...
function isNftMetadataWebPageUrl(url){
  try{
    const u=new URL(String(url||""));
    const host=u.hostname.toLowerCase();
    // explorer.apertum.io/token/.../instance/... sowie ?tab=metadata sind HTML-UI-Seiten.
    // Sie sind weder JSON-Metadatenendpunkte noch browserseitig CORS-freigegeben.
    if(host==="explorer.apertum.io" && (/^\/token\//i.test(u.pathname)||u.searchParams.has("tab")))return true;
    return false;
...
async function fetchExternalNftMetadata(uri){
  ...
  try{
    ...
    if(contentType && !contentType.includes("json") && !contentType.includes("text/plain"))return null;
    const j=await res.json();
      ...
      <div class="project-data-table nft-modern-table" style="margin:0;border:0;border-radius:0;max-height:720px">
        ...
        ${visible.map(n => {
          ...
          const firstOwned=own?.known
            ...
            : "Besitzhistorie offen";
          const walletSince=own?.known?nftOwnershipDate(own.walletSinceAt):"Besitzhistorie offen";
