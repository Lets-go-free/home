// Phase 6.07 · 24.09.2026 12:12:33 CEST: Audit P3 Fresh-Read-Readback: Ownership-Mutationen erzwingen einen neuen DB-Readback; P3 bleibt bis Retest in Arbeit. Build 20260924-121233.
...
const LIFECYCLE_ARCH_AUDIT_ITEMS = [
  ...
  {priority:"P3", workStatus:"in Arbeit", severity:"critical", area:"Fresh-Build-Parität", finding:"Praxistests 6.04–6.06: 20 NFT-Kandidaten werden gefunden und 20 Ownership-Änderungen ohne Fehler geschrieben, dennoch meldet die unmittelbar folgende Konsistenzprüfung weiterhin exakt 20 Lücken. Damit liegt der verbleibende Widerspruch im Write→Readback-/Runtime-Cache-Pfad, nicht in der Evidence-Discovery.", action:"Phase 6.07 führt einen erzwingbaren Ownership-Fresh-Read ein. Nach Mutationen wird ein laufender älterer Ownership-Read zuerst abgewartet und danach zwingend eine neue DB-Query gestartet; die Konsistenzprüfung arbeitet erst auf diesem Readback. Der Fresh-Build protokolliert writtenPeriods, changedNfts, totalRows und walletRows. P3 bleibt bis Retest in Arbeit; erst bei ownershipConsistency=complete wird P4 freigegeben."},
...
function renderHardcodingAudit(){
  ...
  el.innerHTML=`
    <div class="custom-token-card" style="margin-bottom:16px">
      <h3 style="margin-top:0">Lifecycle-/Architektur-Audit · Phase 6.07</h3>
