/* WalletTracking Phase 5.51 · 20.09.2026 03:45:30 CEST · Build 20260920-034530 */
...
const ADMIN_SYSTEM_TREE = [
  ...
  {id:"dao-team",level:2,label:"Team",status:"in_progress",start:"–",daily:"–",open:"🟢 IDB + Version",manual:"🟡 On-chain Update",details:[
    ...
    ["DAO Wallet-Team + Partner-Bot-Lifecycle","RAM + Dashboard-Summary","dao1_old_tree_* + aptmdao_tree_* + dao_partner_bot_lifecycle_cache","Apertum RPC/Explorer nur bei Tree-Update bzw. gezielten Partnerdetails","Phase 5.50: Root-Erkennung ist zusätzlich von aktuellen Projekt-Balances entkoppelt; Ownership wird vor Dashboard-/Tree-Cache geladen und der NFT-Fallback prüft alle User-Wallets. Standardansicht ist wallet-zentriert (1 Wallet = 1 Partner). DAO1-/APTMDAO-DIDs desselben Wallets werden nur in der Darstellung zusammengefasst; die on-chain Graphen bleiben getrennt. Liegt dieselbe Partner-Wallet in beiden eigenen Downlines, hat APTMDAO für die Position Vorrang. Neue MinerBot-Käufe lesen die verwendete APTMDAO-DID direkt aus dem Kaufaufruf (verifizierter Referenzfall #31722: DID #7315, Parent #23); historische Ownership bleibt Fallback. Eigene Wallets sind aus Letzte Partneraktivitäten ausgeschlossen. Migration 065."],
