...
function renderGeneralHelp(){
  ...
  el.innerHTML=`<div class="section-title">❓ Hilfe / Handbuch</div>
      ...
      <div class="custom-token-card"><h3 style="margin-top:0">📊 Dashboard, Wallet-Ansicht &amp; Aktualisierung</h3><p class="note">Das Dashboard ist die Startseite. Der zentrale Filter „Wallet-Ansicht“ zeigt Wallets aller Personen, nur eigene Wallets oder die Wallets einer bestimmten erfassten Person. Dieser Filter verändert nur die Darstellung; Discovery- und Aktualisierungslogik werden dadurch nicht fachlich eingeschränkt. Beim Login werden Bestände und Kurse ausschließlich aus dem gespeicherten Stand dargestellt. Ein fehlender oder älterer Preis-Cache startet keine Preis-API, keinen Pool-RPC und keine Projekt-Discovery. Manuelle Snapshots, allgemeiner Discovery-Cache, Gebühren-Summary, NFT-Cache und TLN/VOW-Discovery werden erst beim Öffnen ihres Bereichs geladen. „Daten aktualisieren“ beziehungsweise „Preise aktualisieren“ bleiben bewusste manuelle Aktionen.</p></div>

      <div class="custom-token-card"><h3 style="margin-top:0">🧮 Dashboard-Kennzahlen</h3><p class="note"><strong>Standardmäßig zählen nur eigene Wallets.</strong> Als verwaltet beziehungsweise fremd markierte Wallets werden erst über „Wallets aller Personen“ oder die konkrete Person einbezogen. <strong>Gesamtvermögen</strong> ist die Summe aller mit einem gespeicherten Kurs bewertbaren freien und gebundenen Bestände im gewählten Bereich. <strong>Frei verfügbar</strong> ist der bewertbare Anteil, der laut vorhandenem Stand nicht als gestakt oder anderweitig gebunden ausgewiesen ist. <strong>Aktuell gebunden</strong> wird nur aus einem bereits vorhandenen, belegten Positionscache berechnet; fehlt dieser Nachweis, zeigt das Dashboard bewusst „–“ statt einer geschätzten Null. Unbewertete Token werden sichtbar gemeldet und nicht still in die Geldsumme eingerechnet.</p></div>
      ...
      <div class="custom-token-card"><h3 style="margin-top:0">💰 Kurse</h3><p class="note">Die aktuelle Kursaktualisierung wird zentral über „Preise aktualisieren“ gesteuert. Administratoren wählen in der Token-Tabelle über „Im Dashboard anzeigen“, welche Tokenkurse auf dem Dashboard erscheinen. Projektkurse werden nur zusammen mit einer tatsächlich vorhandenen Projekt-Kachel gezeigt. Anzeige-Kommastellen sind von den technischen Token-Decimalseinstellungen getrennt. Historische Bewertungen verwenden weiterhin ihre eigene historische Preislogik.</p></div>
